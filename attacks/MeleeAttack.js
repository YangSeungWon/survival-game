    ...
    giveDamage() {
        ...
        if (target instanceof Phaser.Physics.Arcade.Group) {
            ...
            target.getChildren().forEach((enemy) => {
                const angleToTarget = Phaser.Math.Angle.Between(this.owner.x, this.owner.y, enemy.x, enemy.y);
            ...
            const angleToTarget = Phaser.Math.Angle.Between(this.owner.x, this.owner.y, target.x, target.y);
            let angleDiff = Math.abs(Phaser.Math.Angle.Wrap(angleToTarget - attackAngle));
